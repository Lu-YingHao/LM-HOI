...
class SetCriterion(nn.Module):
    ...
    def __init__(self, matcher, weight_dict, eos_coef, losses, enable_focal_loss=False, focal_alpha=0.5,
                 focal_gamma=0.2, consider_all=False, distillation_temperature=4.0, 
                 soft_loss_weight=0.7, hard_loss_weight=0.3, enable_feature_buffer=False,
                 buffer_size=1000, feature_dim=512):
        ...
        self.enable_feature_buffer = enable_feature_buffer
        
        # 初始化特征缓存
        if enable_feature_buffer:
            from models.feature_buffer import AdaptiveDistillation
            self.adaptive_distillation = AdaptiveDistillation(
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        if self.enable_focal_loss:
            ...
            focal_loss = self.binary_focal_loss_with_logits(src_logits[idx], labels, reduction='sum',
    ...
    def loss_confidences(self, outputs, targets, indices, num_boxes, log=True):
        ...
        target_classes_o = torch.cat(
            [torch.ones(len(J), dtype=torch.int64, device=box_scores.device) for t, (_, J) in zip(targets, indices)])
    ...
    def loss_simple_distillation(self, outputs, targets, indices, num_boxes):
        """简单蒸馏loss - 验证用"""
        # 从模型中获取中英文特征
        model = outputs.get("model", None)
        if model is None:
            return {"loss_simple_distillation": torch.tensor(0.0, device=outputs["logits_per_hoi"].device)}
        
        # 获取中英文特征
        cn_features = getattr(model, 'cn_features', None)
        en_features = getattr(model, 'en_features', None)
        
        if cn_features is None or en_features is None:
            return {"loss_simple_distillation": torch.tensor(0.0, device=outputs["logits_per_hoi"].device)}
        
        # 计算简单的MSE蒸馏loss
        distillation_loss = F.mse_loss(cn_features, en_features)
        
        return {"loss_simple_distillation": distillation_loss}
    
    def loss_teacher_student_distillation(self, outputs, targets, indices, num_boxes):
        """Teacher-Student蒸馏loss"""
        # 从模型中获取中英文特征
        model = outputs.get("model", None)
        if model is None:
            return {"loss_teacher_student_distillation": torch.tensor(0.0, device=outputs["logits_per_hoi"].device)}
        ...
        # 中文特征作为teacher（detach，不计算梯度）
        teacher_features = cn_features.detach()
        
        # 英文特征作为student
        student_features = en_features
        
        # 软标签蒸馏参数
        temperature = self.distillation_temperature
        
        # 计算logits（如果特征维度太高，先降维）
        if teacher_features.dim() > 2:
            ...
            teacher_features_flat = teacher_features
            student_features_flat = student_features
        
        # 软标签蒸馏
        teacher_logits = teacher_features_flat / temperature
        student_logits = student_features_flat / temperature
        ...
        # 硬标签蒸馏（MSE）
        hard_loss = F.mse_loss(student_features, teacher_features)
        
        # 基础蒸馏loss
        base_loss = self.soft_loss_weight * soft_loss + self.hard_loss_weight * hard_loss
        
        # 如果启用特征缓存，添加跨batch信息
        if self.enable_feature_buffer:
            total_loss, _stats = self.adaptive_distillation.compute_adaptive_loss(
                cn_features, en_features, base_loss
            )
            # 仅返回用于优化的 Tensor，避免reduce_dict堆叠非Tensor类型
            return {"loss_teacher_student_distillation": total_loss}
        else:
            return {"loss_teacher_student_distillation": base_loss}
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {
            ...
            'simple_distillation': self.loss_simple_distillation,
            'teacher_student_distillation': self.loss_teacher_student_distillation,
        }
