...
class HOIResidualAttentionBlock(nn.Module):
    ...
    def forward(self, image: torch.Tensor, hoi: torch.Tensor, mask: torch.Tensor = None,
                prompt_hint: torch.Tensor = torch.zeros(0, 768)):
        ...
        x = x + self.dropout3(
            self.hoi_attention(self.hoi_ln1(x), attn_mask=self.parse_attn_mask[1:, 1:].to(hoi.device)))
        hoi = x[:hoi_length]
...
class HOIVisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if enable_dec:
            ...
            self.image_patch_pos = nn.Parameter(
                scale * torch.randn((self.image_resolution // self.patch_size) ** 2 + num_tokens, width))
        ...
        if self.semantic_query:
            ...
            self.image_patch_pos2 = nn.Parameter(
                scale * torch.randn((self.image_resolution // self.patch_size) ** 2, width))
            ...
            self.multi_region_attention = TransformerDecoder(decoder_layer2, num_layers=2, norm=decoder_norm2,
    ...
    def interpolate_pos_embedding(self, x, mask):
        ...
        interploated_pos_embed = torch.cat(
            [class_pos_embed + torch.zeros(bs, 1, dim).type_as(x), interploated_pos_embed], dim=1)
...
class CNAdapter(nn.Module):
    ...
    def forward(self, cn_embedding):
        return self.proj(cn_embedding)
...
class HOIDetector(nn.Module):
    def __init__(
        ...
        interaction_decoder_layer = TransformerDecoderLayer(d_model=embed_dim, nhead=8, dim_feedforward=1024,
                                                            dropout=0.1, activation='relu', normalize_before=False)
        interaction_decoder_norm = nn.LayerNorm(embed_dim)
        ...
        self.adapter = CNAdapter(self.embed_dim)
        self.initialize_parameters()
    ...
    def encode_text(self, text, cn_embedding, pure_words=False, is_auxiliary_text=False):
        ...
        cn_mapped = self.adapter(cn_embedding.float())
        q1 = x.unsqueeze(0)  # (1,B,D)
        m1 = cn_mapped.unsqueeze(0)  # (1,B,D)
        f_en2cn = self.interaction_decoder(tgt=q1, memory=m1).squeeze(0).squeeze(0)  # (B,D)

        q2 = cn_mapped.unsqueeze(0)  # (1,B,D)
        m2 = x.unsqueeze(0)  # (1,B,D)
        f_cn2en = self.interaction_decoder(tgt=q2, memory=m2).squeeze(0).squeeze(0)  # (B,D)

        fused = 0.5 * (f_en2cn + f_cn2en)
        x = x + self.alpha * fused + self.beta * cn_mapped
        return x
    ...
    def text_to_embedding(self, text, pure_words=False):
        ...
        if pure_words:
            ...
            for action_token, object_token in text:
                remain_length = self.context_length - self.prefix_length - self.conjun_length - len(action_token) - len(
                    object_token)
                if remain_length < 0:
                    raise RuntimeError(f"Input text is too long for context length {self.context_length}")
                eot_indices.append(self.context_length - remain_length - 1)
        ...
        eot_indices = torch.as_tensor(eot_indices)
        if len(all_token_embeddings) == 0:
            print(all_token_embeddings)
        x = torch.stack(all_token_embeddings, dim=0)  # [batch_size, n_ctx, d_model]

        return x, eot_indices

    def forward(self, image, text, cn_embedding, image_mask, img_sizes):

        if self.use_prompt_hint:
            prompt_hint = self.encode_text(text, cn_embedding, pure_words=True)
        ...
        if self.clip_preprocess:
            resized_img = [torchvision.transforms.Resize([self.input_resolution, self.input_resolution])(
                image[i][:, :img_sizes[i, 0], :img_sizes[i, 1]]) for i in range(bs)]
        ...
        feature_maps = self.dgw_weights_s3[0] * image_0 + self.dgw_weights_s3[1] * image_1 + self.dgw_weights_s3[
            2] * image_2

        vision_outputs = self.hoi_visual_decoder(feature_maps, mask=decoder_mask, prompt_hint=prompt_hint)
        text_features = self.encode_text(text, cn_embedding)
